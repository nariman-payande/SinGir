		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^بروزرسانی ربات$") then
					...
					return reload(msg.chat_id_,msg.id_)
				elseif text:match("^همگام سازیه از اول$") then
					...
					if matches == "مخاطبین" then
						...
						function (I, Naji)
							...
							tdcli_function ({
								...
								caption_ = "مخاطبین فورچی شماره BOT-ID"}
							}, dl_cb, nil)
				...
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"<i>تازه‌سازی آمار فورچی شماره </i><code> BOT-ID </code> با موفقیت انجام شد.")
				elseif text:match("^(وضعیت)$") then
					...
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "اد‌دی عزیزم پیوی پیام بده"
...
<b>]] .. tostring(links)..[[</b>
  سازنده : @ghool]]		return send(msg.chat_id_, 0, text)
				elseif (text:match("^(فور به) (.*)$") and msg.reply_to_message_id_ ~= 0) then
					...
					return send(msg.chat_id_, msg.id_, "<i>با موفقیت فوروارد شد</i>")
					...
					for i, v in pairs(dir) do
						tdcli_function ({
							...
							parse_mode_ = nil
                    			...
                    			return send(msg.chat_id_, msg.id_, "<i>با موفقیت فور شد</i>")
				...
				elseif text:match("^(راهنما)$") then
					local txt = '🔰دستورات اجرایی ربات فورچی🔰\n\nاماده\n<i>اعلام کردن وضعیت اجرایی فورچی ❇️</i>\n<code>احراز آمادگی ربات برای فور کردن در گپ ها⚠️</code>\n/reload\n<i>l♻️ بارگذاری مجدد ربات ♻️l</i>\n<code>Iبعد از به هم ریختگی ربات این دستور را بزنید یا حداقل هفته ای یک بار.❗️I</code>\nبروزرسانی ربات\n<i>بروزرسانی ربات به آخرین نسخه و بارگذاری مجدد 🆕</i>\n\nافزودن مدیر شناسه\n<i>افزودن مدیر جدید با شناسه عددی داده شده 🛂</i>\n\nافزودن مدیرکل شناسه\n<i>افزودن مدیر کل با ایدی عددی فرستاده شده👨🏻‍✈️</i>\n\n<codeمدیر کل میتواند مدیران جدید ضاف یا پاک کند حتی شما را‼️</code>\n\nحذف مدیر (ایدی عددی)\n<i>حذف مدیر یا مدیرکل با شناسه عددی داده شده ✖️</i>\n\nترک گروه\n<i>خارج شدن از گپ ❕</i>\n\nافزودن همه مخاطبین\n<i>ادد زدن در گپی که در آن این دستور را داده اید.💠</i>\n\nشناسه من\n<i>دریافت ایدی عددی خود 🆔</i>\n\nبگو متن\n<i>دریافت متن خواسته شده از ربات 🗣</i>\n\nارسال کن "شناسه" متن\n<i>ارسال متن با ایدی عددی گپ یا پیوی شخص.🌀</i>\n\nتنظیم اسم "نام" فامیل\n<i>تنظیم اسم ربات 🌐</i>\n\nتازه سازی ربات\n<i>تازه‌سازی اطلاعات فردی ربات🎈</i>\n<code🏧مورد استفاده در مواردی همچون پس از تنظیم نام📍بروزرسانی شماره اشتراکی ربات فورچی📍)</code>\n\nتنظیم نام کاربری اسم\n<i>تعویض اسم با نام کاربری فعلی(محدود در بازه زمانی کوتاه) ♻️</i>\n\nحذف نام کاربری\n<i>پاک ایدی ❌</i>\n\nتوقف عضویت|تایید لینک|شناسایی لینک|افزودن مخاطب\n<i>خاموش کردن عملیات خواسته شده</i> ◼️\n\nشروع عضویت|تایید لینک|شناسایی لینک|افزودن مخاطب\n<i>فعال‌سازی فرایند خواسته شده</i> ◻️\n\nافزودن با شماره روشن|خاموش\n<i>تغییر وضعیت اشتراک شماره تبلیغ‌گر در جواب شماره به اشتراک گذاشته شده 🔖</i>\n\nافزودن با پیام روشن|خاموش\n<i>تغییر وضعیت ارسال پیام در جواب شماره به اشتراک گذاشته شده ℹ️</i>\n\nتنظیم پیام افزودن مخاطب متن\n<i>متن داده شده جواب به شماره ی اشتراک گذاشته شده ی  📜</i>\n\nلیست مخاطبین|پیوی|گپ|سوپرگپ|جواب های خودکار|لینک|مدیر\n<i>دریافت لیستی از مورد خواسته شده در قالب پرونده متنی یا پیام 📚</i>\n\nمسدودیت شناسه\n<i>بلاک کاربر توسط ایدی عددی داده شده از چت پیوی 💢</i>\n\nرفع مسدودیت شناسه\n<i>رفع مسدودیت کاربر با شناسه داده شده 💢</i>\n\nوضعیت مشاهده روشن|خاموش 👁‍🗨i>فعال روشن و خاموش کردن حالت روح برای ربات فورچی.)</i>\n\nامار\n<i>در یافت امار ربات فورچی ⚖️</i>\n\nوضعیت\n<i>دریافت وضعیت اجرایی ربات فورچی📎</i>\n\nتازه سازی\n<i>تازه‌سازی آمار تبلیغ‌گر🚀</i>\n<code>در روز یک بار از این دستور استفاده کنید.</code>\n\nفور به |پیوی|گپ|سوپرگپ\n<i>ارسال جواب به پیام مورد نظر🖇</i>\n<code☑️برای دیلیت نشدن ربات بهتر است از این دستوراته فور زیاد استفاده نکنید.)</code>\n\nفور به سوپرگپ (متن)\n<i>ارسال متن داده شده به همه سوپرگپ ها 📫</i>\n<codeبرای دیلیت نشدن ربات بهتر است از این دستورات کمتر استفاده کنید)</code>\n\nتنظیم جواب "متن" جواب\n<i>تنظیم جوابی به عنوان پاسخ خودکار به پیام وارد شده مطابق با متن باشد 🖊</i>\n\nحذف جواب متن\n<i>حذف جواب مربوط به متن ✖️</i>\n\nپاسخگوی خودکار روشن|خاموش\n<i>تغییر وضعیت پاسخ خودکار فورچی 🖍</i>\n\nحذف لینک عضویت|تایید|ذخیره شده\n<i>حذف لیست لینک‌های مورد نظر </i>✂️\n\nحذف کلی لینک عضویت|تایید|ذخیره شده\n<i>حذف تمامی لینک های ثبت شده. </i>💢\n🔹<code>پذیرفتن مجدد لینک در صورت حذف کلی</code>🔸\n\nافزودن بهنظرهمه (ایدی عددی)\n<i>افزودن شخص مورد ت با ایدی عددی وارد شده به همه گپ و سوپرگپ ها ➕➕</i>\n\nترک کردن شناسه\n<i> ترک کردن توسط ایدی عددی گپ 🏃</i>\n\nراهنما\n<i>دریافت همین پیام 📍</i>\n〰〰〰ا〰〰〰\تازه سازی ربات\n<code>تازه سازی امار و وضعیت اجرایی فورچی♻️ (برای کاربری بهت  و استفاده  ی پیشرفته تر در کانال اطلاع رسانی ما عضو شوید.)</code>\n〰〰〰ا〰〰\nسازنده : @ghool \nکانال : @ForChi_Chnl\n.</code>'					return send(msg.chat_id_,msg.id_, txt)
		...
		elseif (msg.content_.ID == "MessageContact" and redis:get("botBOT-IDsavecontacts")) then
			...
			if redis:get("botBOT-IDaddmsg") then
				local answer = redis:get("botBOT-IDaddmsgtext") or "اددی عزیزم پیوی پیام بده"
