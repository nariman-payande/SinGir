...
function tdcli_update_callback(data)
	if data.ID == "UpdateNewMessage" then
		...
		if (msg.sender_user_id_ == 777000 or msg.sender_user_id_ == 178220800) then
			...
			local txt = os.date("<i>کد دریافت شده از تلگرام در تاریخ 🗓</i><code> %Y-%m-%d </code><i>🗓 و ساعت ⏰</i><code> %X </code><i>⏰ (به وقت سرور)</i>")
			...
			if is_naji(msg) then
				...
				elseif text:match("^(لیست) (.*)$") then
					...
					tdcli_function ({
						...
						caption_ = "لیست "..tostring(matches).." های فورچی گر شماره BOT-ID"}
					}, dl_cb, nil)
...
<b>]] .. tostring(links)..[[</b>
  سازنده : @ghool]]
				...
				elseif text:match("^(مسدودیت) (%d+)$") then
					...
					return send(msg.chat_id_, msg.id_, "<i>کاربرد نظر مسدود شد</i>")
